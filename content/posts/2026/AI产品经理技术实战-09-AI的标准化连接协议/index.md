...
但有一个根本问题：**每个工具都需要单独对接API。**

如果你的AI产品需要对接10个不同的系统（CRM、ERP、工单系统、邮件系统……），你需要写10套不同的对接代码。每增加一个系统，就要加一套代码。

**MCP（Model Context Protocol）就是要解决这个问题的标准协议。**

学习完本章后，你将能够：

1. **理解MCP诞生的背景**——为什么API生态需要标准化
2. **掌握MCP的三个核心抽象**——Resources、Prompts、Tools
3. **理解Server与Client的关系**——谁提供能力、谁消费能力
4. **区分本地MCP和远程MCP**——架构差异对用户体验的影响
5. **了解MCP生态现状**——已有哪些可用Server
6. **为客户工单系统设计MCP Server**——暴露内部系统能力
